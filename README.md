...
x86-64 有 16 个 64 位通用寄存器和 16 个浮点数寄存器，此外 CPU 还有一个 64 位的指令指针寄存器 rip，它存放了将要执行的下一条指令的地址。还有其他一些我们可能用不到的寄存器。16 个通用寄存器组除了从 IA32 的 64 位扩展寄存器：rax、rcx、rdx、rbx、rsi、rdi、rsp 和 rbp，还有新增加的 8 个寄存器 r8 - r15。
